...
double timestampToMs(int64_t micros) {
...
struct JobContext {
    int N; int K; 
    const FlatGeo* fg; const vector<GLayer>* gr;
...
void worker_generic(JobContext ctx) {
    ...
    auto dfs = [&](auto&& self, int l, uint32_t id, int col) -> void {
        if constexpr (TASK == TaskType::ADJACENCY) if (ctx.all_adj_found->load(memory_order_relaxed)) return;

        if (l == 0) {
            col = (fmod((*ctx.fg)[0][2 * id], 86400000.0) < 43200000.0) ? 0 : 1;
        ...
        double px = (*ctx.fg)[l][2 * id], py = (*ctx.fg)[l][2 * id + 1];
        ...
        if (l == N - 1) {
            if constexpr (TASK == TaskType::FARTHEST) {
                ...
                // LEAF PRUNING: Instantly reject points bounded by current max
                double min_d = 1e30;
                ...
                for (int c = 0; c < K; ++c) {
                    ...
                    if (d <= current_max) { valid = false; break; } // O(1) Amortized check!
                    if (d < min_d) min_d = d;
                }
            ...
            // BOUNDING & PRUNING LOGIC
            if constexpr (TASK == TaskType::FARTHEST) {
                if (K > 0) {
                    bool pruned = false;
                    double current_max = u64_to_dbl(g_mx_bits.load(memory_order_relaxed));
                    
                    // TREE PRUNING: Stop exploring subtree if ANY center guarantees d <= max
                    for (int c = 0; c < K; ++c) {
                        double dx = px - cx[l*K+c], dy = py - cy[l*K+c];
                        double v = (pre ? pre[c] : 0.0) + dx*dx + dy*dy;
                        cur[c] = v;
                        if (bptr && (v + bptr[(l+1)*K+c] <= current_max)) {
                            pruned = true; break; // O(1) Amortized Branch Cutoff!
                        }
                    }
                    if (pruned) return; 
                }
                ...
                for(int c=0; c<K; ++c) {
                    ...
                    double v = (pre ? pre[c] : 0.0) + dx*dx + dy*dy;
                    cur[c] = v;
                    if (ctx.sexes[c] == ctx.fromG) {
                        if (v < min_lb_target) min_lb_target = v;
                    } else {
                        double ub = v + (bptr ? bptr[(l+1)*K+c] : 0);
                ...
                if (min_lb_target >= current_min) return; // Mathematical B&B Minimization
            }

            uint32_t s = (*ctx.gr)[l].off[id], e = (*ctx.gr)[l].off[id+1];
            for (uint32_t k = s; k < e; ++k) self(self, l + 1, (*ctx.gr)[l].tgt[k], col);
        }
    };
    ...
    while (true) {
        ...
        for (size_t i = s; i < e; ++i) dfs(dfs, 0, (uint32_t)i, -1);
    }
}

// --------------------------- EXACT WORKFLOW (PARALLEL) ---------------------------
...
pair<Point, bool> findAnyJoinTuple(int N, const FlatGeo& f, const vector<GLayer>& gr, int target_color) {
    Point wk(2 * N);
    function<bool(int,int)> dfs = [&](int l, int i) -> bool {
        if (l == 0 && target_color != -1) {
            int c = (int)(fmod(f[0][2 * i], 86400000.0) < 43200000.0 ? 0 : 1);
            if (c != target_color) return false;
        }
        wk[2*l]=f[l][2*i]; wk[2*l+1]=f[l][2*i+1];
        if (l == N - 1) return true;
        uint32_t st = gr[l].off[i], en = gr[l].off[i+1];
        for (uint32_t x = st; x < en; ++x) if (dfs(l+1, gr[l].tgt[x])) return true;
        return false;
    };
    for (size_t i = 0; i < f[0].size()/2; ++i) if (dfs(0, (int)i)) return {wk, true};
    return {wk, false};
}
...
// --------------------------- APPROX WORKFLOW (FAST SEQUENTIAL) ---------------------------
...
CoresetResult run_approx_workflow(const FlatGeo& fg, const vector<GLayer>& gr, int k, double eps, bool prop) {
    ...
    auto dfs = [&](auto& self, int l, uint32_t id, uint64_t state, int col) -> void {
        double px = fg[l][2*id], py = fg[l][2*id+1];
        if (l == 0) col = (fmod(px, 86400000.0) < 43200000.0) ? 0 : 1;
        
        uint64_t nstate = (state << 8) | nearest_c[l][id];
        p_coords[2*l] = px; p_coords[2*l+1] = py;
        ...
        if (l == N - 1) {
            ...
            if (!memo[l].insert(nstate ^ ((uint64_t)id << 32) ^ col)) return;
            uint32_t s = gr[l].off[id], e = gr[l].off[id+1];
            for (uint32_t j = s; j < e; ++j) self(self, l+1, gr[l].tgt[j], nstate, col);
        }
    };
    for(size_t i=0; i<fg[0].size()/2; ++i) dfs(dfs, 0, (uint32_t)i, 0, -1);
...
// --------------------------- NEW LOAD FUNCTION ---------------------------
void load_parquet(const string& fn, Relation& g, vector<KeyPair>& k) {
    // Open the file
    ...
    if (!s.ok()) {
        cerr << "Failed to read Parquet table from " << fn << endl;
        exit(1);
    }
    ...
    if (pickup_idx == -1 || dropoff_idx == -1 || pu_idx == -1 || do_idx == -1) {
        cerr << "Required columns not found in " << fn << endl;
        exit(1);
    }
    
    // Extract arrays (assuming no chunks, or we take first chunk)
    ...
    for (int64_t i = 0; i < num_rows; ++i) {
        // Extract pickup day to filter (keep only day 17)
        int64_t pickup_micros = pickup_arr->Value(i);
        time_t rt = pickup_micros / 1000000;
        struct tm *ptm = gmtime(&rt);
        if (ptm->tm_mday != 17) continue;   // keep only trips on the 17th
        
        double pickup_ms = timestampToMs(pickup_micros);
        double dropoff_ms = timestampToMs(dropoff_arr->Value(i));
        
        g.push_back({pickup_ms, dropoff_ms});
        k.emplace_back(static_cast<double>(pu_arr->Value(i)), static_cast<double>(do_arr->Value(i)));
    }
    
    cout << "Loaded " << g.size() << " trips from " << fn << endl;
}

// --------------------------- MODIFIED MAIN ---------------------------
int main() {
    int N = 4;  // we have four files
    int k = 30;
    
    vector<Relation> g(N);
    vector<vector<KeyPair>> ky(N);
    
    // List of Parquet files (adjust paths as needed)
    vector<string> files = {
    ...
    // Load each file into its own relation
    for (int i = 0; i < N; ++i) {
        load_parquet(files[i], g[i], ky[i]);
    }
    
    // Build index structures (unchanged from original code)
    vector<FastKeyMap> fki(N); 
    for(int i=1;i<N;i++) {fki[i]=FastKeyMap(ky[i].size()); for(size_t x=0;x<ky[i].size();x++) fki[i].ins((i%2!=0?ky[i][x].first:ky[i][x].second), (uint32_t)x);}
    FlatGeo fg(N); for(int i=0;i<N;i++) for(auto& p:g[i]){fg[i].push_back(p[0]); fg[i].push_back(p[1]);}
    vector<GLayer> gr(N-1); 
    for(int l=0;l<N-1;l++){gr[l].off.push_back(0); for(size_t i=0;i<ky[l].size();i++){
        double kv=(l%2==0?ky[l][i].first:ky[l][i].second); const auto* t=fki[l+1].get(kv); if(t)gr[l].tgt.insert(gr[l].tgt.end(),t->begin(),t->end()); gr[l].off.push_back((uint32_t)gr[l].tgt.size());}}
    
    auto t1 = chrono::high_resolution_clock::now(); 
    auto res = run_approx_workflow(fg, gr, k, 0.45, true); 
