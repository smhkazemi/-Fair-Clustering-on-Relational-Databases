...
class StarEngine {
public: // Keeping it accessible for explicit materialized looping verification mapping!
    vector<Hub> hubs;
    RNG rng;
...
vector<MatPoint> MaterialiseJoin(const StarEngine& engine) {
    int rel_limit = 4;
    vector<MatPoint> M;
    for (const auto& h : engine.hubs) {
        long long local_n = h.t1s.size();
        if (local_n == 0) continue;
        long long total_combs = local_n * local_n * local_n * local_n;
        vector<int> idx(4, 0);
        ...
        for (long long i = 0; i < total_combs; ++i) {
            ...
            p.reserve(8);
            for(int d = 0; d < 4; ++d) {
                p.push_back(h.t1s[idx[d]]);
                p.push_back(h.t2s[idx[d]]);
            }
            int color = h.colors[idx[3]]; 
            Scale(p);
            M.push_back({p, color});
            
            for(int d = 3; d >= 0; --d) {
                idx[d]++;
                if (idx[d] < local_n) break;
                idx[d] = 0;
            }
        }
    }
...
int main() {
    ...
    bool proportional = false; double eps = 0.05; 
    vector<int> k_values = {6, 12, 18, 24, 30};
    
    vector<Row> raw; Load("flights.tsv", raw); if (raw.empty()) return 1;

    // DETERMINISTIC SEARCH TO EXACTLY YIELD ~10,000 JOIN SIZE
    mt19937 prng(42); 
    ...
    while (low <= high) {
        ...
        unordered_map<double, int> temp_counts;
        for (int i = 0; i < mid; ++i) temp_counts[shuffled[i].k]++;
        long long cur_size = 0;
        for (const auto& kv : temp_counts) cur_size += (long long)kv.second * kv.second * kv.second * kv.second;
    ...
    StarEngine db; db.build(sampled_raw);
    auto mat_join = MaterialiseJoin(db);

    cout << "\n-----------------------------------" << endl;
    cout << "Original DB lines      : " << raw.size() << endl;
    cout << "Sampled relations limit: " << sampled_raw.size() << endl;
    cout << "Materialized join size : " << mat_join.size() << " records" << endl;

    for (int k_goal : k_values) {
        cout << "\n-----------------------------------" << endl;
        cout << "Running Config for k = " << k_goal << endl;
        ...
        for (auto& l : S12) {
            for (auto& r_p : S34) {
                Point p = l; p.insert(p.end(), r_p.begin(), r_p.end()); 
                X_root.push_back(p);
            }
        }
        ...
        cout << "Points in Coreset: " << Coreset.size() << " | Approx. Alg Runtime: " 
             << chrono::duration_cast<chrono::milliseconds>(t2 - t1).count() << " ms." << endl;
        cout << "Final Fair k-Median Approx Cost on Materialised Data: " << approx_cost << endl;
        ...
        cout << "Baseline Run Exact Cost                             : " << base_cost 
             << " | Baseline Time: " << chrono::duration_cast<chrono::milliseconds>(tb2 - tb1).count() << " ms." << endl;
    }
