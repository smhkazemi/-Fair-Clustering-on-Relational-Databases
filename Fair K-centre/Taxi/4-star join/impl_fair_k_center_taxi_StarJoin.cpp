        ...
        if (l == N - 1) {
            if constexpr (TASK == TaskType::FARTHEST) {
                ...
                for (int c = 0; c < K; ++c) {
                    ...
                    if (d <= current_max) { valid = false; break; }
                    if (d < min_d) min_d = d;
                }
            ...
            if constexpr (TASK == TaskType::FARTHEST) {
                ...
                if (min_lb_target >= current_min) return;
            }
...
void load_parquet(const string& fn, Relation& g, vector<KeyPair>& k) {
    ...
    if (!s.ok()) { cerr << "Failed to read " << fn << endl; exit(1); }
    ...
    if (pickup_idx == -1 || dropoff_idx == -1 || pu_idx == -1 || do_idx == -1) {
        cerr << "Required columns not found in " << fn << endl; exit(1);
    }
...
int main() {
    ...
    // Build indexes: for each relation (1..N-1) index on PULocationID (first of KeyPair)
    vector<FastKeyMap> fki(N);
    for (int i = 1; i < N; ++i) {
        fki[i] = FastKeyMap(ky[i].size());
        for (size_t x = 0; x < ky[i].size(); ++x) {
            fki[i].ins(ky[i][x].first, (uint32_t)x);   // always use PULocationID
        }
    }
    ...
    // Build star join graph as a chain using only PULocationID
    vector<GLayer> gr(N-1);
    for (int l = 0; l < N-1; ++l) {
        gr[l].off.push_back(0);
        for (size_t i = 0; i < ky[l].size(); ++i) {
            double kv = ky[l][i].first;   // PULocationID of current tuple
            const auto* t = fki[l+1].get(kv);
            if (t) gr[l].tgt.insert(gr[l].tgt.end(), t->begin(), t->end());
            gr[l].off.push_back((uint32_t)gr[l].tgt.size());
        }
    }
    
    // Approximate workflow (sequential)
    auto t1 = chrono::high_resolution_clock::now();
    auto res = run_approx_workflow(fg, gr, k, 0.45, true);
    auto t2 = chrono::high_resolution_clock::now();
    
    vector<Point> S_app = res.red_centers;
    S_app.insert(S_app.end(), res.blue_centers.begin(), res.blue_centers.end());
    ...
    // Exact workflow (parallel, branch‑and‑bound)
    auto t3 = chrono::high_resolution_clock::now();
    auto r_e = solveExactFairOnJoin(N, fg, gr, (int)res.red_centers.size(), (int)res.blue_centers.size());
    auto t4 = chrono::high_resolution_clock::now();
    
    vector<Point> S_exe = r_e.first;
    S_exe.insert(S_exe.end(), r_e.second.begin(), r_e.second.end());
