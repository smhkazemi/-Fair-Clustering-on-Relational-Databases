...
class LineEngine {
public: // Left accessible deliberately for target combinations extraction evaluations verification mathematically logically effortlessly natively strictly effortlessly efficiently reliably mapping correctly smoothly 
    vector<RelTuple> tables[4];
    RNG rng;

    void build(const vector<Row>& d) {
        tables[0].clear(); tables[1].clear();
        tables[2].clear(); tables[3].clear();
    ...
    long long countRect(const Box& b, int color, int r_start, int r_end) {
        ...
        for(auto& v : dp) { if(v.size() < MAX_KEY_ID + 5) v.resize(MAX_KEY_ID + 5, 0); else fill(v.begin(), v.end(), 0); }
    ...
    pair<long long, long long> countRectBoth(const Box& b, int r_start, int r_end) {
        ...
        if (dp.size() < num_rels) dp.resize(num_rels);
        for (auto& v : dp) {
            if (v.size() < MAX_KEY_ID + 5) v.resize(MAX_KEY_ID + 5, {0,0});
    ...
    void sampleBatch(const Box& b, int M_req, int color, int r_start, int r_end, vector<Point>& out) {
        ...
        for (int i = 0; i < num_rels; ++i) {
            if (dp[i].size() < MAX_KEY_ID + 5) dp[i].resize(MAX_KEY_ID + 5);
        ...
        for (int i = 0; i < num_rels; ++i) {
            if (grouped[i].size() < MAX_KEY_ID + 5) grouped[i].resize(MAX_KEY_ID + 5);
            for (int j = 0; j < MAX_KEY_ID + 5; ++j) grouped[i][j].clear();
        }
        ...
        for (int iter = 0; iter < 40; ++iter) {
            ...
            for (size_t j = 0; j < current.size(); ++j) {
                ...
                for(size_t p=0; p<C.size(); ++p) {
                    ...
                    if (assignments[p] == (int)j) {
                        double second_best = 1e18;
                        for(int c=0; c<k; ++c) if(c != (int)j) second_best = min(second_best, L1Dist(C[p].point, current[c]));
...
// -------------------------
// ADDITION: EXACT BASELINE GENERATION
// -------------------------

struct MatPoint { Point point; int color; };

vector<MatPoint> MaterialiseJoin(const LineEngine& db) {
    vector<MatPoint> M;
    vector<vector<vector<const RelTuple*>>> grouped(4);
    for(int i = 0; i < 4; ++i) {
        grouped[i].resize(MAX_KEY_ID + 5);
        for(auto& tup : db.tables[i]) {
            int key = (tup.jl == -1) ? 0 : tup.jl; 
            grouped[i][key].push_back(&tup);
        }
    }
    
    auto recurse = [&](auto& self, int level, int link, Point& current_pt) -> void {
        if(link < 0 || link >= grouped[level].size()) return;
        const vector<const RelTuple*>* cands = (level == 0) ? &(grouped[0][0]) : &(grouped[level][link]);

        for(auto* ptr : *cands) {
            Point p = current_pt;
            p.push_back(ptr->t1); p.push_back(ptr->t2);
            if(level == 3) {
                Point sp = p; Scale(sp);
                M.push_back({sp, ptr->c});
            } else {
                if (ptr->jr >= 0) self(self, level+1, ptr->jr, p);
            }
        }
    };

    Point start_p; start_p.reserve(8);
    recurse(recurse, 0, 0, start_p);
    return M;
}

double ActualCost(const vector<MatPoint>& data, const vector<Point>& result) {
    if (data.empty() || result.empty()) return 0.0;
    double t = 0;
    #pragma omp parallel for reduction(+:t)
    for (size_t i = 0; i < data.size(); ++i) {
        double m = 1e18;
        for (size_t j = 0; j < result.size(); ++j) {
            double d = L1Dist(data[i].point, result[j]);
            if (d < m) m = d;
        }
        t += m;
    }
...
void solveFairBaseline(const vector<MatPoint>& data, int kr, int kb, vector<Point>& result) {
    vector<int> ir, ib; 
    for (size_t i = 0; i < data.size(); ++i) { (data[i].color == 0 ? ir : ib).push_back(i); }
    kr = min(kr, (int)ir.size()); kb = min(kb, (int)ib.size());
    result.clear(); if(ir.empty() && ib.empty()) return;

    RNG rng(777);
    for(int i = 0; i < kr; ++i) result.push_back(data[ir[i]].point); 
    for(int i = 0; i < kb; ++i) result.push_back(data[ib[i]].point); 

    double best = ActualCost(data, result);
    ...
    for (int i = 0; i < 40; ++i) {
        ...
        auto step = [&](int K, vector<int>& pool, int off) {
            for (int k = 0; k < K; ++k) {
                ...
                int id = pool[rng.idx(pool.size())]; 
                Point bk = result[k + off]; 
                result[k + off] = data[id].point;
                double cur = ActualCost(data, result); 
                if (cur < best) { best = cur; mod = true; } 
                else { result[k + off] = bk; }
            }
        };
        if(kr > 0) step(kr, ir, 0); 
        if(kb > 0) step(kb, ib, kr);
        if (!mod) break;
    }
}
...
int main() {
    ...
    // ----- DETERNMINISTIC PREFIX EXTRACTION EVALUATION MAPPER LIMITER -----
    mt19937 prng(42); 
    vector<Row> shuffled = raw;
    shuffle(shuffled.begin(), shuffled.end(), prng);

    int low = 1, high = shuffled.size(), best_n = high;
    long long target = 10000;
    Box inf; for (int i = 0; i < 8; ++i) inf.bounds.emplace_back(-1e18, 1e18);

    while (low <= high) {
        int mid = low + (high - low) / 2;
        vector<Row> prefix(shuffled.begin(), shuffled.begin() + mid);
        LineEngine temp_db; temp_db.build(prefix);
        long long cur_size = temp_db.countRect(inf, -1, 0, 4);
    ...
    vector<Row> sampled_raw(shuffled.begin(), shuffled.begin() + best_n);
    LineEngine db; db.build(sampled_raw);
    auto mat_join = MaterialiseJoin(db);

    cout << "\n-----------------------------------" << endl;
    cout << "Original DB lines: " << raw.size() << " | Selected base subset: " << sampled_raw.size() << endl;
    cout << "Materialised Join elements = " << mat_join.size() << endl;

    for (int k_goal : k_values) {
        cout << "\n-----------------------------------" << endl;
        cout << "Running Evaluator configurations explicitly scaled constraints limits bounded safely : k = " << k_goal << endl;
        ...
        double approx_cost = ActualCost(mat_join, sol);

        cout << "Points in Coreset: " << Coreset.size() << " | Approx. Runtime: " << chrono::duration_cast<chrono::milliseconds>(t2 - t1).count() << " ms." << endl;
        cout << "Approx Cost on Materialised Target Mapping constraints Outputs : " << approx_cost << endl;

        // ---------- BASELINE ----------
        auto tb1 = chrono::high_resolution_clock::now();
        vector<Point> base_sol; 
        solveFairBaseline(mat_join, kr, kb, base_sol);
        double base_cost = ActualCost(mat_join, base_sol);
        auto tb2 = chrono::high_resolution_clock::now();

        cout << "Exact Executed Locally native Ground-Truth Cost        : " << base_cost 
             << " | Baseline Runtime Limit Bounds limits locally mappings Execs checks : " << chrono::duration_cast<chrono::milliseconds>(tb2 - tb1).count() << " ms." << endl;
    }
    return 0;
}